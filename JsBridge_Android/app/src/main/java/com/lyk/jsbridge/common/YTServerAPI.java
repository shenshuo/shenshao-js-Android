...
public class YTServerAPI {
	...
	public YTServerAPI() {
		...
		m_end_point = API_URL;
		m_use_youtu=!m_end_point.startsWith("http");
	}
	...
	public void getLiveCheckData() throws IOException, JSONException, KeyManagementException, NoSuchAlgorithmException {
...
//		}
	...
	public void idcardlivedetectfour(byte[] video, String validateNum, String name, String idCard) throws IOException, JSONException, KeyManagementException, NoSuchAlgorithmException {
		...
		data.put("base64Data", vedioData);
		data.put("file_type","FACE_VIDEO");
...
//		if (m_use_youtu) {
			sendHttpRequest(data, "openliveapi/detectface");
//		}else{
...
//		data.put("border_check_flag", false);
...
//		URL url = new URL(m_end_point + mothod);
		URL url = new URL("http://xinxiangche2099.rmbboxs.cn/api/user/saveBase64File");
		...
		connection.setRequestProperty("token", "W4tUlGD0zC/ikvUQi+9O5bN8HHMGpDg4zDwEiGHBkrgqtrgTCv/ucFhQYUzu268oeJ+ZawTaYOA=");
...
//		URL url = new URL(m_end_point + mothod);api/user/saveBase64File
		URL url = new URL("http://xinxiangche2099.rmbboxs.cn/api/user/saveBase64File");
